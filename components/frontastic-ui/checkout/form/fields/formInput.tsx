...
const FormInput = ({
    ...
    let labelElement = <label htmlFor={name} className={labelClassNames ? labelClassNames : "block text-sm font-medium text-gray-700"}>
        {label}
    </label>

    let inputElement = <div className={inputContainerClassNames ? inputContainerClassNames : "mt-1"}>
        <input
            ...
            onChange={(e: React.FormEvent) => onChange((e.target as HTMLInputElement).name, (e.target as HTMLInputElement).value)}
            value={value}
            checked={inputType === "checkbox" ? value === "true" : undefined}
        />
    </div>

    return <div className={containerClassNames ? containerClassNames : "col-span-full"}>
